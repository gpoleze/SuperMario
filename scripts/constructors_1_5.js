...
function CharacterPack() {
    ...
    this.whichFrame = function () {
        ...
        //Face Left
        if (this.velX < 0)
            this.direction = this.directionList[1];

        //Jump
        if (this.velY > 0 || this.velY < 0)
            this.action = this.actionList[2];
    ...
    this.collide = function (obj) {
        ...
        var upRight = {
            x: that.x + that.width * that.scale,
            y: that.y,
            qx: Math.floor((that.x + that.width * that.scale) / 8),
            qy: Math.floor(that.y / 8),
        };
        var bottomLeft = {
            x: that.x,
            y: that.y + that.height * that.scale,
            qx: Math.floor(that.x / 8),
            qy: Math.floor((that.y + that.height * that.scale) / 8),
        };
        var bottomRight = {
            x: that.x + that.width * that.scale,
            y: that.y + that.height * that.scale,
        ...
        //        console.log(obj); receiving right
        console.log("\n\nx and y: ", this.x, this.y);
        console.log("velX and velY: ", this.velX, this.velY);
        console.log("width and height: ", this.width, this.height);

        console.log("qx and qy: ", bottomLeft.qx, bottomLeft.qy);
        console.log("obj[qx][qy]: ", obj[bottomLeft.qx][bottomLeft.qy]);


        //Moving right
        if (this.vx >= 0) {
            if (obj[upRight.qx][upRight.qy]) {
                this.velX = 0;
            } else if (obj[bottomRight.qx][bottomRight.qy]) {
                this.velX = 0;
            }
        }
        ...
        //Moving left
        if (this.vx >= 0) {
            if (obj[upLeft.qx][upLeft.qy]) {
                this.velX = 0;
            } else if (obj[bottomLeft.qx][bottomLeft.qy]) {
                this.velX = 0;
            }
        }
        ...
        if (this.vy <= 0) {
            ...
            if (obj[upLeft.qx][upLeft.qy]) {
                this.velY = 0;
            } else if (obj[upRight.qx][upRight.qy]) {
                this.velY = 0;
            }
        }
        ...
        if (this.velY >= 0) {
            console.log("v >= 0");
            if (obj[bottomLeft.qx][bottomLeft.qy]) {
                this.velY = 0;
                console.log("inside bottoem left");
            } else if (obj[bottomRight.qx][bottomRight.qy]) {
                this.velY = 0;
                console.log("inside bottoem rigth");
                console.log("inside bottoem rigth");
            }
        }
    }
